...
export class SwipeGestureHandler {
	readonly SWIPE_ACTIVATION_DISTANCE: number;
	readonly DIRECTIONALITY_THRESHOLD: number;
	readonly DEACTIVATION_THRESHOLD: number;
	readonly COLLAPSE_SPEED: number;
	readonly ANIMATE_SPEED: number;
	readonly SPRING_BACK_SPEED: number;
	readonly SNAP_SPEED: number;
	readonly REVEAL_WIDTH: number;
	touchState: TouchState;
	rafId: number | null;
	activatedWrapper: HTMLElement | null;
	...
	activateRow(wrapper: HTMLElement) {
	...
	deactivateRow(wrapper: HTMLElement) {
		if (!wrapper) return;
		wrapper.classList.remove('activated');
		const content = wrapper.querySelector<HTMLElement>('.swipe-content');
		if (content) {
	...
	_onTouchStart(e: TouchEvent) {
		...
		const wrapper = swipeContent.closest<HTMLElement>('.thread-row-wrapper');
		if (
			!wrapper ||
