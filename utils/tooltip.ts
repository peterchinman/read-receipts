...
window.addEventListener(
	...
	(e) => {
		if (e.pointerType === 'touch') {
			_touchActive = true;
			clearTimeout(_touchTimer ?? undefined);
...
export function initTooltips(
	...
	const textEl = layer.querySelector<HTMLElement>('.tooltip-text');
	const hotkeyEl = layer.querySelector<HTMLElement>('.tooltip-hotkey');
	const subtextEl = layer.querySelector<HTMLElement>('.tooltip-subtext');
	...
	tooltipElements.forEach((element) => {
		element.addEventListener('mouseenter', (e) => {
			if (!_touchActive) showTooltipForElement(e.currentTarget as HTMLElement);
		});
