...
function renderPieceView(container: HTMLElement, pieceId: string | undefined) {
	container.innerHTML = `
...
function renderVerifyPage(container: HTMLElement, token: string | undefined) {
	container.innerHTML = `<auth-verify token="${token}"></auth-verify>`;
}

function renderAdminLogin(container: HTMLElement) {
...
function renderCreateView(container: HTMLElement) {
 ...
 */
function navigateToThread(thread: Thread | null | undefined) {
 ...
 */
async function handleEditParam(editId: string, editToken: string | null) {
	try {
		if (!editToken) return;
