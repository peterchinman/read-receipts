class MultiSwitch extends HTMLElement {
	...
	attributeChangedCallback(name, oldVal, newVal) {
		...
		if (name === 'value') {
			const idx = this._options.findIndex((o) => o.value === newVal);
	...
	connectedCallback() {
		...
		const idx = this._options.findIndex((o) => o.value === val);
	...
	#render() {
		...
		requestAnimationFrame(() => {
			...
			const container = this.shadowRoot.querySelector('.multi-switch');
			if (!container) return;
		...
		labels.forEach((label) => {
			...
			label.addEventListener('keydown', (e) => {
				...
				switch (e.key) {
					...
					case ' ':
						e.preventDefault();
						this.#select(parseInt(label.dataset.index, 10));
						break;
					case 'ArrowRight':
					case 'ArrowDown':
						e.preventDefault();
	...
	#select(index, moveFocus = false) {
		...
		// Update radio
		const radio = this.shadowRoot.querySelector(`#opt-${index}`);
	...
	#updateThumb(selectedIndex) {
		const thumb = this.shadowRoot.querySelector('.switch-thumb');
		const container = this.shadowRoot.querySelector('.multi-switch');
