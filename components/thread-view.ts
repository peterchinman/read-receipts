...
class ThreadDisplay extends HTMLElement {
	...
	private _focusMessageRafId: number | null = null;
	private _iosGradientSyncRafId: number | null = null;
	private _inputObserver: ResizeObserver | null = null;
	private _headerObserver: ResizeObserver | null = null;
	private _shrinkWrapInitialized: boolean = false;
	...
	setMessages(messages: MessageLike[]) {
	...
	renderAdd(message: MessageLike, messages: MessageLike[]) {
	...
	renderUpdate(message: MessageLike, messages: MessageLike[]) {
		if (!message) return;
		if (Array.isArray(messages)) this._messages = messages;
		if (message.id) this.#removeMessageNodes(message.id);
		const nodes = this.#insertMessageNodesAtIndex(message, messages);
		this._shrinkWrapFor(nodes);
		this.#refreshConsecutiveAround(nodes);
		this.#refreshDateSeparators();
	...
	renderDelete(message: MessageLike) {
		if (!message) return;
	...
	renderReset(messages: MessageLike[]) {
		this._messages = Array.isArray(messages) ? messages : [];
	...
	#renderRecipient(recipient: RecipientLike) {
	...
	#createBubbleRow(message: string, sender: string, id: string) {
	...
	#renderMessageNodes(m: MessageLike | null) {
		const nodes = [];
		const sender =
	...
	#findFirstNodeByMessageId(id: string) {
		const container = this.$.messageList;
	...
	#insertMessageNodesAtIndex(message: MessageLike, messages: MessageLike[]) {
		const container = this.$.messageList;
	...
	#renderAll(messages: MessageLike[]) {
		const messageList = this.$.messageList;
		if (!messageList) return;
	...
	#prevMessageRow(row: Element): HTMLElement | null {
	...
	#nextMessageRow(row: Element): HTMLElement | null {
	...
	#isConsecutiveRow(prevRow: HTMLElement, currRow: HTMLElement) {
	...
	#applyConsecutive(row: HTMLElement, prevRow: HTMLElement | null) {
	...
	#refreshConsecutiveAround(nodes: HTMLElement[]) {
	...
	#getDateKey(timestamp: string | undefined) {
	...
	#formatDateLabel(timestamp: string, showYear = false) {
		const d = new Date(timestamp);
	...
	#createDateSeparator(timestamp: string, showYear = false) {
	...
	_shrinkWrapFor(nodes: HTMLElement[]) {
		...
		this._shrinkWrapForRafId = requestAnimationFrame(() => {
			for (const el of nodes) {
				...
				if (el && el.classList && el.classList.contains('message-row')) {
					bubbles = Array.from(el.querySelectorAll<HTMLElement>('.bubble'));
				}
	...
	#applyShrinkWrapToBubble(bubble: HTMLElement) {
	...
	#unwrapShrinkWrapForBubble(bubble: HTMLElement) {
	...
	_scheduleIOSGradientSync() {
		...
		this._iosGradientSyncRafId = requestAnimationFrame(() => {
			...
			const selfBubbles = list.querySelectorAll<HTMLElement>(
	...
	_debounce(func: (...args: unknown[]) => void, wait: number) {
