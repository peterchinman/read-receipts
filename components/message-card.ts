...
export class MessageCard extends HTMLElement {
	...
	#updateDateDisplay(dateInput: HTMLInputElement) {
	...
	#updateExactDateDisplay(input: HTMLInputElement) {
	...
	_onInput(e: Event) {
		...
		if (target.matches('textarea')) {
			this.#resizeTextarea(target as HTMLTextAreaElement);
			this.#emit('editor:update', {
				id: this.messageId,
				patch: { message: (target as HTMLTextAreaElement).value },
			});
			...
			this.#updateExactDateDisplay(target as HTMLInputElement);
		}
	}
	...
	_onTouchStart(e: Event) {
		...
		this._touchStartY = te.touches[0]?.clientY ?? 0;
	}
	...
	_onTouchEnd(e: Event) {
		...
		const deltaY = Math.abs(
			(te.changedTouches[0]?.clientY ?? 0) - this._touchStartY,
		);
		if (deltaY > 10) return;

		const path = te.composedPath();
		const target = (path[0] ?? te.target) as HTMLElement;
